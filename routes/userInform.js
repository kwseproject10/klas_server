...
router.get("/", (req, res) => {
  ...
  const query =
    "select u1.userName, u1.userType,m.majName as major,u1.userID as ID,(select count(*) from (select distinct lecYear, lecSem from enrollments as e join lectures as l on e.lecKey = l.lecKey where e.userID = u1.userID) as yearSem) as numberOfTerm,u1.email,u1.phone as phoneNum,u1.birth as birthday,u2.userName as advisor,u2.email as advisorEmail,u2.tel as advisorNum,u1.userState as state , ufName from users as u1 left join users as u2 on u1.advID = u2.userID join majors as m on u1.majID = m.majID left join userfiles uf on u1.userID = uf.userID where u1.userID = ?";
  /*
